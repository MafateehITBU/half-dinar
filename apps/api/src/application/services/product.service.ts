    ...
    const product = await prisma.$transaction(async (tx) => {
      const created = await tx.product.create({
        data: {
          ...
          nameAr: input.nameAr,
          nameEn: input.nameEn,
          descriptionAr: input.descriptionAr ? (await import('../../shared/sanitize-html.js')).sanitizeRichText(input.descriptionAr) : input.descriptionAr,
          descriptionEn: input.descriptionEn ? (await import('../../shared/sanitize-html.js')).sanitizeRichText(input.descriptionEn) : input.descriptionEn,
          price: input.price,
